...
Code for the paper. This is an anonymized copy prepared for review: author,
institution, repository and archive identifiers are withheld.
...
DATASHEET.md        Datasheet for the raw trajectory archive.
```
...
hidden-state models form a separate archive (roughly 3 TB), prepared for
public release with the datasheet in [`DATASHEET.md`](DATASHEET.md). Its
identifier is withheld during anonymous review. The Codeforces
`official_tests` column (used by `scripts/run_cot_analysis.py` for code
evaluation) is in the same archive.
...
Withheld during anonymous review. The three upstream datasets
(Easy2Hard-Bench, Hendrycks MATH, SATBench) are cited in
